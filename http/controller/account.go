...
func (a AccountController) Register(ctx echo.Context) error {
	...
	// 不验证邮箱，注册完成直接登录
	// 自动登录
	xhttp.SetLoginCookie(ctx, username)
...
func (a AccountController) SendActivateEmail(ctx echo.Context) error {
	...
	if uuid != "" {
		...
		go logic.DefaultEmail.SendActivateMail(user.Email, helper.RegActivateCode.GenUUID(user.Email), isHttps)
	}
...
func (AccountController) Login(ctx echo.Context) error {
	...
	// 登录成功，种cookie
	xhttp.SetLoginCookie(ctx, userLogin.Username)
...
func (AccountController) Logout(ctx echo.Context) error {
	...
	session.Save(xhttp.Request(ctx), xhttp.ResponseWriter(ctx))
	// 重定向得到原页面
